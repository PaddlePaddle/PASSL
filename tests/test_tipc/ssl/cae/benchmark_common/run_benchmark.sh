...
function _set_params(){
    ...
    PRETRAIN_CHKPT=${9:-'pretrained/cae/cae_base_patch16_224_8k_vocab_pretrained_800ep.pd'}
    accum_iter=${10:-1}
    # 以下为通用执行命令，无特殊可不用修改
...
function _train(){
    ...
    ft) echo "run run_mode: ${mode}_${run_mode}"
        train_cmd="python -m paddle.distributed.launch --nnodes=1 --master=127.0.0.1:12538 \
                    ...
                    --print_freq 1 --max_train_step ${max_iter} --data_path ./dataset/ILSVRC2012/ \
                    --output_dir ./output/finetune_ep100_fp16o1 --model ${model} --finetune ${PRETRAIN_CHKPT} \
                    --nb_classes 1000 --batch_size ${global_batch_size} --lr 8e-3 --accum_iter ${accum_iter} \
                    --warmup_epochs 5 --epochs 100 --layer_decay 0.65 --drop_path 0.1 --weight_decay 0.05 \
                    --mixup 0.8 --cutmix 1.0 --sin_pos_emb --dist_eval --no_auto_resume --exp_name finetune_ep100_fp16o1 "  
        workerlog_id=0
        ;;
    ...
    pt) echo "run run_mode: ${mode}_${run_mode}"
        train_cmd="python -m paddle.distributed.launch --nnodes=1 --master=127.0.0.1:12538 \
                    ...
                    --print_freq 1 --max_train_step ${max_iter} --data_path ./dataset/ILSVRC2012/ \
                    --output_dir ./output/ep800_fp16o1 --model ${model} --discrete_vae_weight_path ${PRETRAIN_CHKPT} \
                    --batch_size ${global_batch_size} --lr 1.5e-3 --warmup_epochs 10 --epochs 800 --clip_grad 3.0 \
                    --layer_scale_init_value 0.1 --imagenet_default_mean_and_std --color_jitter 0 --drop_path 0 \
                    --sincos_pos_emb --mask_generator block --num_mask_patches 75 --decoder_layer_scale_init_value 0.1 \
                    --no_auto_resume --save_ckpt_freq 50 --exp_name ep800_fp16o1 --regressor_depth 4 --seed 0 \
                    --num_decoder_self_attention 4 --dual_loss_weight 2 --dual_path_ema 0 "
        workerlog_id=0
        ;;
