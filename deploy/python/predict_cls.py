...
class ClsPredictor(Predictor):
    def __init__(self, config):
        ...
        if "preprocess" in config:
            if "transforms" in config["preprocess"]:
        ...
        if self.benchmark:
            ...
            import os
            pid = os.getpid()
            size = config["preprocess"]["transforms"][1]["CropImage"]["size"]
            self.auto_logger = auto_log.AutoLogger(
                model_name=config.get("model_name", "cls"),
                model_precision='fp16' if config["use_fp16"] else 'fp32',
                batch_size=config.get("batch_size", 1),
                data_shape=[3, size, size],
                save_path=config.get("save_log_path", "./auto_log.log"),
...
def main(config):
    ...
    image_list = get_image_list(config["infer_imgs"])
    ...
    for idx, img_path in enumerate(image_list):
        ...
        if cnt % config["batch_size"] == 0 or (idx + 1) == len(image_list):
            if len(batch_imgs) == 0:
                continue
...
if __name__ == "__main__":
    ...
    config = config.get_config(args.config, overrides=args.override)
    main(config)
