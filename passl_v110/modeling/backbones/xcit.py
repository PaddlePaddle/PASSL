...
# Code was based on https://github.com/facebookresearch/xcit
...
class PositionalEncodingFourier(nn.Layer):
    ...
    def forward(self, B, H, W):
        ...
        dim_t = self.temperature ** (
            2 * dim_t.floor_divide(paddle.to_tensor(2)) / self.hidden_dim)
    ...
    def __init__(self,
        ...
        if eta is not None:     # LayerScale Initialization (no layerscale when None)
            self.gamma1 = self.create_parameter(
                [dim],
                default_initializer=nn.initializer.Assign(eta * paddle.ones([dim])))
...
class XCA(nn.Layer):
    ...
    def __init__(self,
        ...
        self.temperature = self.create_parameter(
            [num_heads, 1, 1],
            default_initializer=nn.initializer.Assign(paddle.ones([num_heads, 1, 1])))
...
class XCABlock(nn.Layer):
    def __init__(self,
        ...
        self.gamma2 = self.create_parameter(
            [dim],
            default_initializer=nn.initializer.Assign(eta * paddle.ones([dim])))
        self.gamma3 = self.create_parameter(
            [dim],
            default_initializer=nn.initializer.Assign(eta * paddle.ones([dim])))
...
class XCiT(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cls_token = self.create_parameter(
            [1, 1, embed_dim],
            default_initializer=nn.initializer.Assign(paddle.zeros([1, 1, embed_dim])))
